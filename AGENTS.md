...
Security release rules: no embedded PowerShell payloads, no runtime script extraction or ExecutionPolicy overrides. Scripts must be visible and hash-bound to the binary. Do not disable Defender, add exclusions, restore quarantined builds, weaken TLS/trust checks, or repack/obfuscate to manipulate detection. Scan the complete distributable before executing/publishing it, retain explicit cloud/real-time limitations, and default releases to prerelease. A normal-channel release requires a version-scoped maintainer approval and documentation of the outstanding security-review limitations; never infer malware clearance from release-channel metadata. Never claim Microsoft clearance without a recorded determination for the exact hash.
