...
func TestHandoffParentProcess(t *testing.T) {
	...
	if path == "" { t.Skip("child-process fixture") }
	...
	err = launchReplacementHelper(script, path, true, 20*time.Second)
	if os.Getenv("CHATGPT_UPDATE_TEST_FAIL") == "1" {
		if err == nil { t.Fatal("invalid stage handoff accepted") }
		fmt.Println("EXPECTED startup failure:", err)
		return
	}
	if err != nil { t.Fatal(err) }
	if err = os.WriteFile(path+".parent-ready", []byte("ready"), 0600); err != nil { t.Fatal(err) }
...
func fixturePlan(t *testing.T) (replacementPlan, string) {
	...
	if err != nil { t.Fatal(err) }
	if _, err = os.Stat(filepath.Join(source, exeAsset)); os.IsNotExist(err) { t.Skip("build.ps1 must run first") }
	...
	for _, dest := range []string{root, candidate} {
		if err = os.MkdirAll(filepath.Join(dest, "scripts"), 0700); err != nil { t.Fatal(err) }
		for _, name := range portableFiles {
			b, e := os.ReadFile(filepath.Join(source, filepath.FromSlash(name)))
			if e != nil { t.Fatal(e) }
			if e = os.WriteFile(filepath.Join(dest, filepath.FromSlash(name)), b, 0600); e != nil { t.Fatal(e) }
		}
	}
...
func TestAutomaticConsoleHandoff(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(root, "README.md"), oldReadme, 0600); err != nil { t.Fatal(err) }
	...
	if err := showLastUpdate(root); err == nil { t.Error("running helper was not reported") }
	if err := cmd.Wait(); err != nil { t.Fatalf("parent fixture: %v\n%s", err, output.String()) }
	...
	for time.Now().Before(deadline) {
		...
		if err == nil && (s.Phase == "success" || s.Phase == "failed") {
			...
			if err != nil || !bytes.Equal(b, oldReadme) { t.Fatal("old package not retained") }
			for _, record := range plan.Files {
				h, _, e := hashFile(filepath.Join(root, filepath.FromSlash(record.Path)))
				if e != nil || h != record.SHA256 { t.Fatalf("component not updated: %s", record.Path) }
			}
			t.Log("actual new-console launch acknowledged, parent exited, replacement verified, rollback copies retained")
			return
		}
...
func TestAutomaticHandoffValidationFailure(t *testing.T) {
	...
	if err != nil { t.Fatalf("parent: %v\n%s", err, b) }
	s, err := readUpdateState(plan.StatusPath)
	if err != nil || s.Phase != "failed" || !strings.Contains(s.Message, "Staged component changed") { t.Fatalf("failure not persisted: %+v %v", s, err) }
	log, err := os.ReadFile(plan.Log)
	if err != nil || !bytes.Contains(log, []byte("[FAIL]")) { t.Fatal("failure absent from log") }
	if _, err = os.Stat(plan.Previous); !os.IsNotExist(err) { t.Fatal("existing package touched before consent handshake") }
	if _, err = os.Stat(filepath.Join(filepath.Dir(path), "handoff-continue")); !os.IsNotExist(err) { t.Fatal("failed validation granted replacement") }
}
...
`
	if err := os.WriteFile(path, []byte(script), 0600); err != nil { t.Fatal(err) }
	...
	if err != nil || result != syscall.WAIT_OBJECT_0 { t.Fatalf("console process: %v %v", result, err) }
	var code uint32
	if err = syscall.GetExitCodeProcess(pi.Process, &code); err != nil || code != 0 { t.Fatalf("console exit: %d %v", code, err) }
	b, err := os.ReadFile(report)
	if err != nil { t.Fatal(err) }
	var streams map[string]bool
	if err = json.Unmarshal(b, &streams); err != nil { t.Fatal(err) }
	if len(streams) != 3 { t.Fatal("missing stream probes") }
	for name, redirected := range streams {
		if redirected { t.Errorf("%s must belong to the new console, not NUL", name) }
	}
}
